...
# Detect privilege-escalation tool (sudo / doas). Root gets a no-op "eval".
if [ "$(id -u)" = "0" ]; then
    ...
    if [ -z "$ESCALATION_TOOL" ]; then
        printf "%b\n" "${RED}Can't find a supported escalation tool (sudo/doas).${RC}"
        exit 1
    fi
fi

# Detect package manager from the same set linutil supports.
PACKAGER=""
